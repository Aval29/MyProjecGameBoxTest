...
public class SpawnerSnowFlake : MonoBehaviour
{
    ...
    private  float Speed()
    {
        float Sp = 1 / ((EventWorld.Temperature / EventWorld.TemperatureMax)  + 0.3f);
    ...
    IEnumerator Spawn()
    {
      ...
      //  for (int i = 0; i < 3 ; i++) /// поиск сводобного места
        {       
        Vector3 pos = spawnPosArea.position + new Vector3(Random.Range(-range.x, range.x), Random.Range(-range.y / 4, range.y), 0);
        var rad = 0.29f; // new Vector3(0, (pos.y + 0.6f), 0) - длина/ширина снежинки 
